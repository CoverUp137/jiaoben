...
new Env('青龙自动备份');
'''
# 目前只测试没v2.11.3之前的青龙版本使用，新版有data文件夹暂不清楚是否能用，不能用就自己改改脚本


# 排除备份的名单，变量名 export QLBK_EXCLUDE_NAMES="  不懂就默认别填
# 备份目标目录 变量名 export QLBK_BACKUPS_PATH=""  默认在/ql/backups 不懂就别乱改，会报错
# 最大备份保留数量默认5个 变量名 export QLBK_MAX_FLIES=""



# 使用教程很简单
# 青龙--依赖管理--python3-安装aligo  如果安装失败看下面安装linux依赖，再装aligo
...
QLBK_EXCLUDE_NAMES = ['log', '.git', '.github',
                      'node_modules', 'backups', '.pnpm-store', 'build', 'cli', 'dist', 'docker', 'public', 'sample', 'shell', 'src', 'repo']   # 填写排除备份的名单
